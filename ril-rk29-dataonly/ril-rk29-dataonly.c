...
static const char* REFERENCE_RIL_VERSION = "RIL_RK_DATA_V9.0"
#ifndef RIL_RELEASE
    " [debug version]"
#endif
#if !ENABLE_DNS_CHECK
...
#define LOG_NDEBUG 0
#define LOG_TAG "RIL_RK_DATA_V9.0"
#include <utils/Log.h>
...
//                  且E1750查询数据状态没问题
	...
	if(!modem_cmp(0x05C6, 0x9000, NULL)) {
		network_type = 11;
	} else if(!modem_cmp(0x05C6, 0x6000, NULL)) {
		network_type = 14;
	}
        asprintf(&responseStr[3], "%d", network_type);
    }
...
static void requestOperator(void *data, size_t datalen, RIL_Token t)
{
    ...
    if(!modem_cmp(0x05C6, 0x9000, NULL) || (!modem_cmp(0x05C6, 0x6000, NULL))) {
